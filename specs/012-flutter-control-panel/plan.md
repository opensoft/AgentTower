...
- **Flutter 3.27 stable** (Dart 3.5+) for the desktop UI, packaged as a single-window app per supported OS. (Historical: the Phase 3 T009 `flutter create` step ran against the bench-global Flutter 3.44.0 because `fvm install 3.27.0` was not viable in the bench environment; see `flutter-testing-plan.md` §"2026-05-23 execution notes". **Bench was re-pinned to 3.27 on 2026-05-25 via T160a** — `pubspec.yaml` ships `intl: ^0.19.0` and `flutter analyze` passes under 3.27.0 with zero errors.)
...
## Cross-cutting widget conventions (post-Phase-8 swarm-review)

The 2026-05-24 swarm review introduced seven cross-cutting helpers
that every feature surface MUST now consume rather than rolling its
own. New features (Phase 9 onward) should reuse them; deviation
needs a documented rationale.

| Helper | Location | Purpose | Source finding |
|---|---|---|---|
| `RuntimeStateGate` + `OutageStateView` / `ContractIncompatStateView` / `DegradedStateView` / `HealthyEmptyStateView` / `LoadingStateView` / `ErrorStateView` | `lib/ui/widgets/runtime_state_views.dart` | FR-004 five-state coverage on every live-data surface; replaces inline `Center(Text('Failed: $err'))` patterns | CR-6 |
| `ContractCheckedButton` | `lib/ui/widgets/contract_checked_button.dart` | FR-002 mutation-disable invariant: every mutation button stays visible-but-disabled with inline tooltip on `contract-version-incompatible` / `runtime-unreachable` | CR-7 |
| `SafeUrlLauncher` | `lib/ui/widgets/safe_url_launcher.dart` | FR-079 / FR-001 daemon-supplied URL scheme allowlist (http/https/mailto direct; file: through confirmation modal; reject javascript:/data:/vbscript:/etc.) | H-D1/H-D2/H-D3 + M-15 |
| `SeverityVisuals` | `lib/domain/severity.dart` | R-15 severity palette + R-22 icon/text/color redundancy triad; consumed by drift, attention, notification severity rendering | CR-8 + H-C3 |
| `withAsOfDefault` | `lib/core/json_utils.dart` | Single canonical `as_of` stamping helper; replaces 4 duplicated `_withAsOf` definitions | H-G3 |
| `MasterClassCapabilities` envelope | `lib/domain/master_qualification.dart` | FR-071 master-class lookup with explicit `degraded` flag + reason; replaces the silent-empty-set degraded path | H-G2 |
| `@JsonEnum(valueField: 'wireValue')` convention | every enum in `lib/domain/models/` | Codegen contract: emits snake_case wire string from each enum's `wireValue` field; without it `fromJson` throws on every multi-word enum | CR-1 |

`MasterSummary.tryFromAgent(...)` MUST be used instead of the raw
freezed factory when constructing a MasterSummary from an
AdoptedAgent-derived shape (H-G1 / data-model.md §3 invariant).

The full swarm-review report + per-batch implementation log live in
`specs/012-flutter-control-panel/swarm-review-2026-05-24.md` and
`specs/012-flutter-control-panel/swarm-review-fix-plan.md`.

## Documentation

The feature directory holds artifacts generated by Spec Kit (`spec.md`,
`plan.md`, `research.md`, `data-model.md`, `contracts/`, `quickstart.md`,
`tasks.md`, `checklists/`). It also retains a few **historical** docs from
the lifecycle that are no longer load-bearing for implementation but are
preserved for audit:

| File | Purpose | Status |
|---|---|---|
| `clarify-questions.md` | Round-1 25-question clarify block | Historical (closed by Clarifications §1) |
| `clarify-questions-f4-f7.md` | Round-2 7-question follow-up on F4/F7 | Historical (closed by Clarifications §2) |
| `clarify-questions-checklist-gaps.md` | Round-3 21-question gap closure | Historical (closed by Clarifications §3 + research R-22..R-42) |
| `codex-prompt-spec-quality-pass.md` | The exact prompt Codex consumed to drive the OpenSpec spec-quality-pass change | Historical (archived as `openspec/changes/archive/2026-05-23-spec-quality-pass-feat-012`) |
| `tier-1-fix-instructions.md` | Pre-Codex tier-1 findings worklist (F1..F12 + F-A1..F-A11) | Historical (all items closed; preserved for audit trail) |
| `flutter-testing-plan.md` | Operator-supplied notes that shaped the test suite layout in plan.md §Project Structure `test/` and `integration_test/` | Reference (not regenerated by Spec Kit) |
| `swarm-review-2026-05-24.md` + `swarm-review-fix-plan.md` | Full report + per-batch implementation log from the 2026-05-24 swarm review (CR-1..CR-8, H-* and M-* findings; see plan.md §"Cross-cutting widget conventions") | Reference (historical audit; remediation items absorbed into tasks T154..T177) |
| `upstream-feat011-extension-draft.md` | Operator hand-off outline for the upstream FEAT-011 v1.1 work that unblocks T160b + T166 + T167 — added 2026-05-25 from /speckit-analyze Round 9 D1 | Draft (promote via `/speckit.specify` on `main` to a new `specs/0NN-feat011-extension-for-feat012/` feature; this file is NOT a Spec Kit artifact) |

The historical files each carry a `> **Status: HISTORICAL**` banner at the
top so a reader who lands on them via search knows they are not the
current source of truth. The `upstream-feat011-extension-draft.md` carries
a `> **Status: DRAFT — operator hand-off outline**` banner for the same
reason.

### Known spec-numbering gap

`spec.md` skips FR-073: the section walks FR-070, FR-071, FR-072 (defined
later under the handoff failure handling block), FR-074, FR-075, … The
gap is intentional in the sense that no requirement currently fills it
and renumbering would invalidate every cross-reference (the
`requirements.md` checklist + plan.md + tasks.md cite FR ids verbatim).
Future additions in this number range MAY claim FR-073 without an
OpenSpec proposal; any other change is a real spec edit and goes through
`/opsx:propose`.
