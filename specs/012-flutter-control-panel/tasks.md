...
**Total tasks**: 177 (T001..T176 with T160 split into T160a + T160b on 2026-05-25; +T173 added 2026-05-25 from test-helper/widget hardening surfacings during the T149/T150/T164 batch; +T174 added 2026-05-25 from architectural surfacings during the T161-T170 integration-test batch (DaemonSession re-bootstrap + mock-daemon `generated_prompt_text` mirror); +T175 added 2026-05-25 from the same batch (broken `open_handoff_flow` palette command); +T176 added 2026-05-25 from the T173 closeout (deferred Fixtures.event vs Event model shape decision)).
**Tasks per phase**: Phase 1 (9) + Phase 2 (44) + Phase 3 US1 (27) + Phase 4 US2 (16) + Phase 5 US3 (15) + Phase 6 US4 (7) + Phase 7 US5 (11) + Phase 8 US6 (13) + Phase 9 (35 — 11 base tasks T143-T153 + 6 verification tasks T154-T159 + 5 post-Phase-3 review follow-ups T160-T164 (T160 split adds +1 ID) + 6 post-Phase-8 follow-ups T165-T170 + 1 post-Round-4 task T171 + 1 post-Round-5 task T172 + 1 post-T149/T150/T164-batch task T173 + 1 post-T161-T170-batch task T174 + 1 post-T161-T170-batch task T175 + 1 post-T173-closeout task T176; 11+6+6+6+1+1+1+1+1+1=35).
...
- [X] T173 [P] **Test-helper + production-widget hardening (consolidated from T149/T150/T164/T161/T162/T163/T165/T170).** Cleanups surfaced across multiple Phase 9 batches. **Done 2026-05-25** — all 8 sub-tasks landed except a single Fixtures.event-shape-alignment carve-out (now deferred as the new T176 below — needs a contract-vs-model decision that's bigger than a polish task).
  - (a) Align `Fixtures.event(...)` in `apps/control_panel/test/helpers/fixture_builders.dart:~363` with the `Event` freezed model shape — the helper currently emits FEAT-011 wire fields (`event_class` / `emitted_at` / `summary`) but the `Event` model expects (`event_type` / `observed_at` / `excerpt`); T164's events-view test had to build its event JSON inline. Also resolves the 3 pre-existing `inference_failure_on_collection_literal` warnings at `fixture_builders.dart:363`. **Done 2026-05-25 (inference warnings only)**: the 3 `inference_failure_on_collection_literal` warnings at line 363 (`'recent': recent ?? const {'events': [], 'queue': [], 'routes': []}` in `Fixtures.dashboardResult`, not `Fixtures.event`) are fixed by adding explicit `<String, List<dynamic>>` + `<dynamic>[]` annotations. The `Fixtures.event` shape alignment is deferred to T176 — it needs a contract-vs-model decision (either rename the Event freezed model fields to match FEAT-011 wire shape, or add @JsonKey mappings, or update Fixtures.event to produce model-shape JSON).
...
- [ ] T174 [P] **DaemonSession re-bootstrap-on-retry + mock-daemon stateful mutations.** Two architectural gaps surfaced by the Phase-9-batch-5 integration tests, both worth a coordinated change rather than test-side workarounds:
  - (a) **Lib-side: "Retry connection" should re-bootstrap the session, not just invalidate the dashboard provider.** T162's outage/recovery test had to manually re-bootstrap `DaemonSession` after tapping "Retry connection" because the existing handler only invalidates the dashboard provider. The session's underlying socket stream was closed by the daemon-kill, so re-fetching from the existing session fails — a real operator would also have to relaunch the app today. Fix: the retry handler should `await ref.read(daemonSessionProvider).reBootstrap()` (or equivalent) before invalidating downstream providers. Affects `lib/features/agent_ops/dashboard/dashboard_view.dart:230,256` retry handlers and any sibling retry buttons.
...
- [ ] T175 [P] **`project_specs.open_handoff_flow` palette command is misnamed.** Per T168's investigation, `apps/control_panel/lib/features/project_specs/module.dart:135` registers a palette command named `open_handoff_flow` whose handler `Navigator.pushNamed`s to `current_work` — it does NOT open `HandoffFlow`. The only in-app surface that opens `HandoffFlow` from regular navigation is `DriftRepairHandoffLauncher` (via `openHandoffFlow`); the palette entry promises something the rest of the UI doesn't deliver. **Two fixes are valid**: (a) rename the palette command to `open_current_work` so the name matches behavior, OR (b) add a real `HandoffFlow` entry-point to the Project and Specs workspace (e.g. a "New handoff" button on Current Work that calls `openHandoffFlow(...)`) — the latter is the more useful UX fix per the spec's FR-036 multi-step flow expectations. Operator decision required.
