...
**Total tasks**: 176 (T001..T175 with T160 split into T160a + T160b on 2026-05-25 during /speckit-implement Phase 9; +T173 added 2026-05-25 from test-helper/widget hardening surfacings during the T149/T150/T164 batch; +T174 added 2026-05-25 from architectural surfacings during the T161-T170 integration-test batch (DaemonSession re-bootstrap + mock-daemon `generated_prompt_text` mirror); +T175 added 2026-05-25 from the same batch (broken `open_handoff_flow` palette command)).
**Tasks per phase**: Phase 1 (9) + Phase 2 (44) + Phase 3 US1 (27) + Phase 4 US2 (16) + Phase 5 US3 (15) + Phase 6 US4 (7) + Phase 7 US5 (11) + Phase 8 US6 (13) + Phase 9 (34 — comprises 11 base tasks T143-T153, plus 6 verification tasks T154-T159 from /speckit-analyze Round 1, 5 post-Phase-3 review follow-ups T160-T164 from /speckit-analyze Round 4 with T160 itself split into T160a + T160b on 2026-05-25 = 6 actual task IDs, 6 post-Phase-8 follow-ups T165-T170 from /speckit-analyze Round 2 covering CR-4 i18n / H-B10 drafted-row / M-11 streaming / H-B1+B2+C2 test rewrites, 1 post-Round-4 task T171 for FR-058 OS-native dispatch wiring, 1 post-Round-5 task T172 for the bench-verified analyze regression gate, 1 post-T149/T150/T164-batch task T173 for test-helper + widget Key hardening, 1 post-T161-T170-batch task T174 for DaemonSession re-bootstrap + mock-daemon `generated_prompt_text` mirror, and 1 post-T161-T170-batch task T175 for the misnamed `open_handoff_flow` palette command, all surfaced 2026-05-25 during /speckit-implement Phase 9; 11+6+6+6+1+1+1+1+1=34).
...
- [ ] T173 [P] **Test-helper + production-widget hardening (consolidated from T149/T150/T164/T161/T162/T163/T165/T170).** Cleanups surfaced across multiple Phase 9 batches:
  - (a) Align `Fixtures.event(...)` in `apps/control_panel/test/helpers/fixture_builders.dart:~363` with the `Event` freezed model shape — the helper currently emits FEAT-011 wire fields (`event_class` / `emitted_at` / `summary`) but the `Event` model expects (`event_type` / `observed_at` / `excerpt`); T164's events-view test had to build its event JSON inline. Also resolves the 3 pre-existing `inference_failure_on_collection_literal` warnings at `fixture_builders.dart:363`.
