...
- [X] T149 [P] Write unit tests under the existing module-based test layout (`test/core/<area>/` and `test/features/<feature>/`, matching the home directory of the code under test — same convention as `test/core/daemon/app_client_test.dart` and `test/features/registry_test.dart`) covering: lifecycle validators (T039-T043) at `test/core/lifecycle/`, envelope parser (T011) at `test/core/daemon/`, error code mapping (T012) at `test/core/daemon/`, UX-state repository (T017) at `test/core/ux_state/` including atomic-write + migration + corruption-quarantine paths, helper-policy resolver (T105) at `test/features/handoff/` including FEAT-011 v1.0 absence fallback (per contracts/helper-policy.md §6). Path-layout aligned with existing convention per /speckit-analyze Round 5 T-N1 (avoid creating a parallel `test/unit/` tree that would compete with the module-based layout). **Done 2026-05-25**: 9 test files, ~2103 lines, 106 test cases, `flutter analyze` passes under 3.27.0 with 0 errors / 0 warnings (26 info-level `require_trailing_commas` nits accepted). Notes: `HelperPolicyResolver` lives at `lib/features/project_specs/handoff/helper_policy_resolver.dart` (not `lib/domain/helper_policy/`, which holds only the freezed model); `MigrationRegistry.migrations` is empty at MVP so the "one test per shipped migration" requirement collapses to an empty-registry assertion that forces a new test when the first migration lands; `PaneStateValidator` is fully connected at MVP (every ordered pair valid) and the test locks this invariant explicitly so future regressions are caught.
- [X] T150 [P] Write widget tests under the existing module-based test layout (alongside the unit tests for each feature), placed by code home: project card rendering (T088) at `test/features/project_specs/widget_project_card_test.dart` with zero/one/two/overflow masters, attention queue (T135) at `test/features/agent_ops/attention/widget_attention_queue_test.dart` for icon+color combinations, severity badges (T028) at `test/ui/widgets/widget_severity_visuals_test.dart` across themes, markdown viewer (T095) at `test/ui/widgets/widget_markdown_viewer_test.dart` for the safe-markdown subset. Path-layout aligned with existing convention per /speckit-analyze Round 5 T-N1 (avoid creating a parallel `test/widget/` tree). **Done 2026-05-25**: 4 widget test files, ~776 lines, ~20 testWidgets, `flutter analyze` passes with no issues. Notes: production attention-queue placeholder copy is "No actionable attention items" (the round-3 clarification's "All clear — no actionable items" wording was never adopted in code; test asserts the real copy). `MarkdownViewer` does NOT itself handle (d) cross-doc `.md` link resolution or (e) missing-path placeholder — per its doc comment those concerns live in the calling surface ("the app never reads doc files itself per FR-001 + R-28"); test documents and skips those scenarios.
- [X] T151 [P] Write golden tests in `apps/control_panel/test/golden/` (alchemist) for: Light/Dark/System themes × Comfortable/Compact densities × every workspace top-level view, ensuring FR-066 WCAG AA contrast and FR-009 density consistency. The `test/golden/` directory is alchemist convention and will be created as part of this task (it does not exist yet — that is not drift, just unrealized future structure, per /speckit-analyze Round 5 T-N1). **Done 2026-05-25**: 444-line `test/golden/workspace_goldens_test.dart` — 4 surfaces (Dashboard, Projects, AvailableValidation, Settings) × 6 variants (3 themes × 2 densities) = 24 captured panes. `alchemist: ^0.11.0` added to dev_dependencies (already pinned by an earlier pubspec edit in this session). Reused `_FakeAppClient` + `_StubRuntimeNotifier` patterns from T164. Each scenario bounded to 1200×800 for cross-machine determinism; `system` theme deterministically mapped to Light so goldens are host-OS-independent. `flutter analyze test/golden/` passes with 0 errors. Operator runs `flutter test test/golden --update-goldens` once to generate baseline images.
...
- [X] T154 [P] Performance verification suite in `apps/control_panel/test/perf/perf_budgets_test.dart` asserting (a) FR-062 cold-start-to-Dashboard p95 ≤ 2 s, (b) FR-063 first-screenful render p95 ≤ 1 s for every FR-063 list view (Containers, Panes, Agents, Events, Queue, Routes, Projects, Available Validation, Runs, Drift) at FEAT-011 default page-size 50, (c) **manual-refresh round-trip** propagation p95 ≤ 2 s from `ref.invalidate` to in-app render. **Note (post-Phase-8 analyze A1, 2026-05-24):** the FR-064 wording was originally "live-update propagation" but no SSE/WebSocket subscription exists yet; T167 below tracks the daemon-side streaming work. Until T167 lands, T154(c) measures manual-refresh round-trip, not push propagation. Closes analyze findings C1+C2+C3. **Done 2026-05-25**: 568 lines, 3 testWidgets, 10 samples per budget per spec R-38; doc comment + testWidgets-title + failure-reason all explicitly call out the pull-vs-push caveat ("T167 lifts this onto push"). FR-062 measures stopwatch around `pumpWidget(AgentTowerControlPanel()) + pumpAndSettle()` with sanity-assert of `'Containers'` text before stopping the clock. FR-063 shares one harness across all 10 lists; project-scoped views override `selectedProjectIdProvider`. FR-064 measures EventsView via `ref.invalidate(eventListProvider)` round-trip. Skip-gated via `isPython3Available()`. `flutter analyze test/perf/` passes with 0 errors.
- [X] T155 Network-trace + subprocess-trace verification in `apps/control_panel/test/security/no_network_no_cli_scrape_test.dart`: run full-workspace exercise under packet capture (assert zero outbound packets except to `releases.opensoft.one`) AND under process trace (assert no subprocess invocation of the `agenttower` CLI). Closes analyze finding C4 / SC-009. **Done 2026-05-25**: 616 lines, 3 test blocks, 0 analyze errors. **Group A (network)** uses paired custom `HttpOverrides` + `IOOverrides` subclasses (`_HttpTraceOverrides`, `_SocketTraceOverrides`) inside `HttpOverrides.runZoned(() => IOOverrides.runZoned(...))` around a full workspace exercise (13 list providers + 3 refreshes). Unix-domain sockets treated as exempt (local-only per FR-001). Asserts the release-feed URL is the canonical `https://releases.opensoft.one/...` and non-HTTPS overrides throw. **Group B (subprocess)** does NOT use `IOOverrides` because `Process` isn't covered by the Dart SDK overrides API; instead does a static source-scan of `lib/**.dart` (skipping `.freezed.dart` / `.g.dart`) for `Process.(run|runSync|start|startSync)('agenttower'` patterns and asserts zero offenders. tcpdump rejected as MVP because it needs CAP_NET_RAW. Documented in-file.
- [X] T156 CLI non-regression smoke in `tests/integration/test_feat012_cli_noop.py` (Python, lives next to existing FEAT-002..010 tests) — assert FEAT-002..010 CLI methods produce byte-identical output before and after the desktop-app build is installed. Closes analyze finding Const2. **Done 2026-05-25**: 2 pytest cases — (1) `test_cli_surfaces_remain_callable_post_feat012` invokes the 6 representative `--json` surfaces (config-doctor, config-paths, agent-list, route-list, queue-list, event-list) against an isolated daemon and asserts each returns exit 0 with parseable JSON; (2) `test_desktop_app_data_dir_does_not_leak_into_cli` snapshots all 6 surfaces, populates the FR-061a `<app-data>/agenttower-control-panel/` directory (XDG + macOS Library variants) with plausible ux-state.json + log files, re-runs all 6 surfaces, asserts byte-identical output. `pytest --collect-only` succeeds; functional run requires a live `agenttower` install + `agenttowerd` (deferred to operator CI lane).
- [X] T157 [P] Codebase audit in `apps/control_panel/tools/audit_no_local_mutation.dart` (or `scripts/`) — static-analysis pass asserting that every mutation in `apps/control_panel/lib/` originates from `core/daemon/app_client.dart` surface (no UI code constructs `app.*` requests directly, no UI code mutates daemon-owned model state). Closes analyze finding C12 / FR-005 invariant. **Done 2026-05-25**: 375-line Dart executable. Three scans: (1) string-literal regex for `'app.<method>'` outside `app_client.dart` (HARD error if any); (2) AST scan for `SocketClient.sendRequest(...)` outside `lib/core/daemon/` (HARD error); (3) Notifier `state =` heuristic for non-`build()` methods (review-severity, doesn't affect exit code — UI-only flow state and stream-handler patterns trigger false-positives). Added `analyzer: ^7.7.1` to dev_dependencies. Tool exits 0 against current `lib/` (zero hard violations; 5 review-severity heuristic flags in onboarding/settings/runtime-state, all expected non-daemon-driven state writes per spot-check). Run: `dart run tools/audit_no_local_mutation.dart`.
...
- [X] T160a [P] **Bench re-pin to Flutter 3.27 (independent — done 2026-05-25).** Originally sub-step (e) of T160; split out per /speckit-implement Phase-9 readiness audit because (e) had no actual dependency on the upstream `app.dashboard` contract bump that gates (a)-(d). Completed: (i) downgraded `apps/control_panel/pubspec.yaml` from `intl: ^0.20.2` → `intl: ^0.19.0` (the documented 3.27 baseline per `flutter-testing-plan.md:154`); (ii) deleted `apps/control_panel/pubspec.lock`; (iii) re-resolved via `/opt/flutter-3.27.0/bin/flutter pub get` — 120 deps resolved cleanly, no other transitive bumps needed adjustment; (iv) `flutter analyze` passes under 3.27.0 with zero errors (94 info/warning lint, all pre-existing style nits). `flutter test` not yet run from the worktree due to stale root-owned `build/unit_test_assets` + `build/linux` directories left by an earlier bench Docker session; sanity-pass deferred to T172 (bench-verified analyze regression gate). Closes the env-blocker that gated Phase 9 Bucket C + Bucket E tasks per the readiness audit. The Phase 3 T009 deviation (T002 known deviation) is now retired in pubspec terms.
- [ ] T160b [P] **Re-enable suppressed FR-012 dashboard tiles + bump contract version (upstream-blocked).** Originally sub-steps (a)-(d) of T160. When the upstream contract bump that extends FEAT-011 `app.dashboard` with `counts.panes.by_state` / `counts.agents.by_state` / `counts.routes.recently_skipped_count` / `recommended_next_action` lands (no such upstream artifact exists yet — neither `specs/014-*` nor any `openspec/changes/extend-app-dashboard-fields-for-feat012` is filed in-tree as of /speckit-analyze Round 6, 2026-05-25; T160b cannot start until one is filed and archived), (a) update `ContractRegistry.declare('agent_ops/dashboard', ...)` in `apps/control_panel/lib/features/agent_ops/module.dart` from `ContractVersion(1, 0)` to `ContractVersion(1, 1)`, (b) un-comment + wire the 4 TODO-marked tiles in `apps/control_panel/lib/features/agent_ops/dashboard/dashboard_view.dart` against the new fields, (c) extend `Fixtures.dashboardResult` in `apps/control_panel/test/helpers/fixture_builders.dart` with optional params for the new fields, (d) update the mock daemon README's `app.dashboard` fixture example. Closes analyze finding C1 + C4.
- [X] T161 [P] **Extend T054 with a real SC-001 walk.** Rewrite `apps/control_panel/integration_test/us1_adopt_and_operate.dart` to actually drive the 8-milestone walk (containers list → pane discovery → adopt → register agent → log attach → direct send → route add) via `tester.tap` + `tester.pumpAndSettle` between each step, then assert the wall-clock elapsed against the SC-001 ≤ 10 minute budget. The current shape pumps the widget + asserts an unmeasured budget — vacuous per the post-Phase-3 analyze C2 finding. Tap-driving requires the build_runner-generated freezed files (see T037 operator note) and `MockDaemonClient` + `Fixtures` for each post-mutation state transition. **Done 2026-05-25**: rewrote to 424 lines (was 148) — `Stopwatch`-wrapped tap-driven walk through all 8 milestones; asserts `< Duration(minutes: 10)` at the end. Mock daemon is fixture-replay only (server.py:207-217 confirmed), so fixture pre-seeds the *union* of pre- and post-mutation states; wire-level state-transition invariant remains in sibling `us1_smoke_walk.dart`. `flutter analyze` passes with 0 issues. Subview ChoiceChips found via `find.ancestor(...matching: find.byType(ChoiceChip))` — adding `Key('subview-chip-$id')` would make the 6×-per-walk lookup more robust (folded into T173).
- [X] T162 [P] **Extend T055 with real SC-010 outage/recovery measurements.** Rewrite `apps/control_panel/integration_test/runtime_states.dart` to (a) start the mock-daemon harness + bootstrap the session, (b) kill the mock daemon process mid-flight and assert every live-data surface transitions to the documented `runtime-unreachable` empty state within 2 s (FR-004 + SC-010), (c) re-spawn the mock daemon + tap "Retry connection" and assert live state reverts within 5 s, (d) assert no surface displays stale data labelled as live during the outage. The current rewrite preserves only the `ContractCompat.compute` matrix test — the wall-clock budgets are unmeasured. Closes analyze finding C2. **Done 2026-05-25**: rewrote to 351 lines (was 68); preserves the `ContractCompat.compute` matrix + 2 helpers and adds the SC-010 outage/recovery `testWidgets`. Uses `MockDaemonClient.socketPathOverride` for a stable Unix socket across kill/respawn. **Two architectural surfacings** (folded into T173 + flagged below): (1) `tester.pumpUntil` does not exist in Flutter 3.27.0 — polyfilled inline as `_pumpUntil`; should be consolidated into `test/helpers/pump_until.dart` since T170 hit the same gap. (2) The dashboard's `_OutageState`/`_DashboardError` uses copy "Retry connection" (matches SC-010 spec wording) but the reusable `OutageStateView` in `lib/ui/widgets/runtime_state_views.dart:120` uses the shorter "Retry" — inconsistent. (3) The lib-level "Retry connection" path only invalidates the dashboard provider; it does NOT re-bootstrap `DaemonSession`. Test had to manually re-bootstrap. Real architectural gap — flagged below.
- [X] T163 [P] **Onboarding milestone auto-tick integration test.** Write `apps/control_panel/integration_test/us1_onboarding_autotick.dart` driving each of the 8 milestones through its trigger condition (mock-daemon emits a container → `benchContainerCheck` ticks; emits a pane → `paneDiscoveryCheck` ticks; etc.) and assert the milestone enters `onboardingProgressProvider`'s persisted set within one pump tick. Verifies the post-Phase-3 `OnboardingProgressNotifier` `ref.watch` wiring (review fix C9) cannot silently regress. Closes analyze finding M2. **Done 2026-05-25**: 665 lines, 8 separate `testWidgets` cases (one per milestone — fast regression pinpoint), 0 issues. **Strategic decision**: bypassed the Python mock-daemon harness entirely (avoids 16 process restarts) and overrode the 6 Riverpod providers `OnboardingProgressNotifier.build()` actually watches (`containerListProvider`, `paneListProvider`, `agentListProvider`, `queueListProvider`, `routeListProvider`, `runtimeStateProvider`). Directly targets M2 (silent regression of `ref.watch` wiring after fix C9). One spec-vs-code ambiguity to verify (folded into T173): `Fixtures.agent()` does not expose a `log_attachment` wire field; test splices it in via `toJson()`. Confirm the real daemon's `AdoptedAgent` payload uses field name `log_attachment` (not `logAttachment` etc.) and that `Fixtures.agent` should be extended to include it natively.
- [X] T164 [P] **Write widget tests for the 12 Phase 3 US1 surfaces** under `apps/control_panel/test/features/agent_ops/` (existing module-based layout — same convention as `test/features/registry_test.dart`; aligned with /speckit-analyze Round 5 T-N1) covering: dashboard_view (with + without runtime-unreachable + with all-zero counts), containers_view (empty + populated + load-error), panes_view (per-PaneState row variants + adopt button visibility), adopt_flow (validation + submit + per-error rendering), agents_view (tree depth + descendants overflow), direct_send (empty-payload guard + send-success snack), log_attach_affordance (attach/detach + busy state), events_view (jump-to-most-recent + empty), queue_view (action visibility per state + delay-on-queued correctness post-H8 fix), routes_view (toggle + remove + recent-skip explanation rendering), add_route_flow (template/target submit), health_view (per-state coloring + hints). Each test pumps with `ProviderScope` + override stubs for `appClientProvider`. Closes analyze finding H13 (deferred from Block D in the review fix-up). **Done 2026-05-25**: 12 widget test files, 1539 lines, 37 testWidgets, `flutter analyze` passes with no issues. Notes: Health view tests assert per-state icon (warning_amber_outlined for degraded) rather than card color since Color equality across theme overrides is brittle.

### Verification tasks (added per /speckit-analyze Round 2, post-Phase-8 / swarm-review)

- [X] T165 [P] **FR-067 i18n sweep — replace ~200 hardcoded English strings.** Audit every Phase 4-8 widget under `apps/control_panel/lib/features/project_specs/**`, `lib/features/agent_ops/attention/**`, `lib/features/notifications/**`, `lib/features/testing_demo/**`, `lib/ui/widgets/markdown_viewer.dart`. Extend `apps/control_panel/assets/l10n/en.arb` with one key per hardcoded `Text('literal')` / `Tooltip(message: '...')` / `SnackBar(content: Text('...'))` / `AppBar(title: Text('...'))` (~200 total). Wire each call-site to `AppLocalizations.of(context).<key>` so adding a new locale becomes a translation + config change with no source-code edits. The localization delegate is already configured in `app.dart` (commented out pending the first sweep) — uncomment + add `AppLocalizations.delegate` after the keys land. Closes swarm-review **CR-4**. **Why now:** FR-067 is unmet in production; this is the single biggest documentation finding from the swarm review. **Done 2026-05-25**: 4 parallel subagents extracted **317 call-sites consolidated to 301 reusable ARB keys (115 placeholders)** across **27 source files**. en.arb grew from 21 → 312 value keys; `app_localizations.dart` regenerated 12 → 312 methods. Delegate uncommented in `lib/app.dart`. `flutter analyze` passes under 3.27.0 with **0 errors** (106 info/warning nits, all pre-existing style or pre-existing inference). Two follow-on integration fixes: (a) `handoff_flow.dart` `DropdownButtonFormField.initialValue:` → `value:` for 3.27.0 compatibility (swarm-review-2026-05-24 note claiming `initialValue` was valid on 3.27+ was wrong — it was verified against the T002 3.44 bench deviation, not the pinned 3.27.0 toolchain; T160a re-pin exposed this); (b) added explicit `AttentionSeverity` / `AttentionClass` type annotations on `attention_queue_view.dart` `_severityRank` and `_classIcon` static helpers (pre-existing implicit-dynamic params that 3.27.0's analyzer flags more strictly). One structural correctness win surfaced by the testing_demo agent: lifted `ReadinessRenderResult.downgradeReason` (English-only) out of `readiness_computation.dart` (no `BuildContext`) into structured `missingRequiredLabels: List<String>` + `branch: String?` fields, with view-side ICU-plural formatting via `demoReadinessDowngradeReasonOne` / `_Many` — avoids broken pluralization in future locales. Note: per-agent import style was inconsistent (notifications used relative path, attention used package: path) — both work; consolidating to one style is a T173 cleanup item. **Scope carve-out (per /speckit-analyze Round 8 E1, 2026-05-25)**: T165's sweep was explicitly limited to the 5 path globs listed above and did NOT cover `lib/features/agent_ops/dashboard/**`, `lib/features/shell/**`, or `lib/ui/widgets/runtime_state_views.dart`. After T165 uncommented `AppLocalizations.delegate` in `app.dart`, those uncovered files began visibly violating FR-067 in production (5× `Text('Retry connection')` literals in `runtime_state_views.dart` + `dashboard_view.dart`, plus "Release notes" / "Version unavailable" in `version_display.dart`). Extension to those paths is tracked separately as T177 below.
...
- [X] T168 [P] [US3] **Rewrite `us3_handoff_flow.dart` for SC-003 real-tap walk.** The current integration test bypasses the widget tree entirely (drives `submitHandoff` against a `ProviderContainer` directly). T097 explicitly forbade that pattern. Drive `tester.tap` + `tester.enterText` to walk: open project → tap "Open handoff flow" → enter range → tap mode → tap Preview → tap Submit. Start a `Stopwatch` at the first tap; assert `< 30 s` at the Submit assertion. Closes swarm-review **H-B1**. **Done 2026-05-25**: rewrote `us3_handoff_flow.dart` to 535 lines, 2 `testWidgets` (SC-003 + SC-004 combined). `Stopwatch` started at the first real tap (work-item entry); assert `< Duration(seconds: 30)` at Submit completion. Material Stepper renders "Continue" as uppercase `CONTINUE` in M2 default on 3.27.0 — `find.text('CONTINUE').first` is correct. Pumps `HandoffFlow` directly via a `MaterialApp` harness because the `project_specs.open_handoff_flow` palette command (`module.dart:135`) is broken — it `Navigator.pushNamed`s to `current_work`, not `HandoffFlow`. Flagged below as a real navigation bug.
- [X] T169 [P] [US3] **Rewrite us3 SC-004 byte-for-byte assertion to be non-tautological.** Today the same `previewText` string is passed to both sides of the equality. Replace with two independent `PromptSkeleton.render()` invocations (one from the preview surface, one captured at submit time) and assert byte equality. Additionally mirror the submitted `generated_prompt_text` into the mock-daemon row so the daemon round-trip is observable + diff-able. Closes swarm-review **H-B2**. **Done 2026-05-25 (partial)**: in the same file as T168; two independent `PromptSkeleton.render()` invocations are now compared (one from `SelectableText` in the preview surface, one from a fresh `PromptSkeleton` constructed from the same source inputs). **The daemon-side mirror sub-clause is NOT closed** — the mock daemon does not echo the submitted `generated_prompt_text` back (server.py:217 deep-copies the fixture template verbatim). Test contains a "soft probe" (`generated_prompt_text isNotEmpty`); the strongest form (compare preview-time render against the actual bytes the daemon stored) requires a mock-daemon extension. Folded into the new T174 below.
- [X] T170 [P] [US4] **Add SC-005 60-second wall-clock budget to `us4_drift.dart`.** T112 explicitly required observing the actual badge transition within the 60 s window. The current test asserts data-flow only. Use `tester.pumpUntil(timeout: const Duration(seconds: 60))` watching the project-card drift badge widget; emit a drift transition from the mock daemon mid-test; assert the badge text updates before the timeout. Closes swarm-review **H-C2**. **Done 2026-05-25**: extended `us4_drift.dart` from 170 → 415 lines (1 testWidgets preserved + 1 new SC-005 testWidgets added). 0 analyze issues. **Clever workaround for stateless mock**: spawns a second mock-daemon process on a second socket with the post-drift fixture, then swaps the in-process `AppClient` pointer via a `_SwappableAppClient` proxy — keeps the widget mounted (no re-pump, no timer reset) so the test measures real wall-clock socket round-trip latency. Drift-badge widget located at `lib/features/project_specs/projects/project_card.dart:373-401` (class `_DriftChip`); polls for the `(1)` open-count suffix as the load-bearing signal. `tester.pumpUntil` polyfilled inline (same gap as T162; consolidation folded into T173).
...
**Total tasks**: 178 (T001..T177 with T160 split into T160a + T160b on 2026-05-25; +T173 added 2026-05-25 from test-helper/widget hardening surfacings during the T149/T150/T164 batch; +T174 added 2026-05-25 from architectural surfacings during the T161-T170 integration-test batch (DaemonSession re-bootstrap + mock-daemon `generated_prompt_text` mirror); +T175 added 2026-05-25 from the same batch (broken `open_handoff_flow` palette command); +T176 added 2026-05-25 from the T173 closeout (deferred Fixtures.event vs Event model shape decision); +T177 added 2026-05-25 from /speckit-analyze Round 8 E1 (FR-067 i18n coverage gap in dashboard/shell/runtime_state_views, exposed when T165 activated the delegate)).
**Tasks per phase**: Phase 1 (9) + Phase 2 (44) + Phase 3 US1 (27) + Phase 4 US2 (16) + Phase 5 US3 (15) + Phase 6 US4 (7) + Phase 7 US5 (11) + Phase 8 US6 (13) + Phase 9 (36 — 11 base tasks T143-T153 + 6 verification tasks T154-T159 + 6 post-Phase-3 review follow-ups (T160a + T160b + T161-T164; T160 split into T160a + T160b adds +1 ID over the original 5) + 6 post-Phase-8 follow-ups T165-T170 + 1 post-Round-4 task T171 + 1 post-Round-5 task T172 + 1 post-T149/T150/T164-batch task T173 + 1 post-T161-T170-batch task T174 + 1 post-T161-T170-batch task T175 + 1 post-T173-closeout task T176 + 1 post-Round-8 task T177; 11+6+6+6+1+1+1+1+1+1+1=36).
...
- T160 was split on 2026-05-25 during /speckit-implement Phase 9 because sub-step (e) had no actual dependency on the upstream-blocked sub-steps (a)-(d):
  - **T160a** [DONE] — bench re-pin to Flutter 3.27 (intl downgrade + lockfile delete + re-resolve); retires the T002 known deviation in pubspec terms. Closes the env-blocker that gated Phase 9 Bucket C + E tasks.
  - **T160b** [BLOCKED] — re-enables suppressed FR-012 dashboard tiles + bumps `agent_ops/dashboard` contract version after the upstream contract bump that extends FEAT-011 `app.dashboard` lands (no such upstream artifact is filed in-tree yet — see T160b body for details).
...
- T160b (formerly T160 sub-steps a-d) — blocked on an upstream contract bump that extends FEAT-011 `app.dashboard` with `counts.panes.by_state` / `counts.agents.by_state` / `counts.routes.recently_skipped_count` / `recommended_next_action`. **No upstream artifact is filed in-tree as of /speckit-analyze Round 6 (2026-05-25)** — neither `specs/014-*` nor any `openspec/changes/extend-app-dashboard-fields-for-feat012/`. T160b cannot start until that upstream change is filed and archived. (T160 was split on 2026-05-25 during /speckit-implement Phase 9; sub-step e — the bench re-pin — became T160a and was completed independently.)
...
- [X] T171 [P] **OS-native dispatch on incoming notifications (FR-058 wiring).** `OsNativeIntegration.consider(notification, enabled: ...)` is defined in `lib/features/notifications/os_native_integration.dart` but has zero callers — Settings persists the FR-058 `osNativeNotifications` toggle but the dispatcher never fires when new notifications arrive. **Implement now using a polling-diff strategy** (Riverpod listener on `notificationListProvider` that diffs the current snapshot against the previous snapshot to identify newly-arrived `incoming`-lifecycle notifications) and call `osNativeIntegrationProvider.consider(notification, enabled: settings.osNativeNotifications)` for each. The dispatcher's internal de-dup + severity-gate handles the rest. **Upgrade path when T167 streaming lands:** swap the polling-diff producer for the streaming subscription's "newly-arrived" events; the consumer side and dispatcher remain unchanged. (T167 is itself blocked on FEAT-011 v1.x, so polling-diff is the production strategy until that lands — not a temporary fallback.) Closes Round-4 analyze C-N1. **Done 2026-05-25**: created `lib/features/notifications/os_native_dispatch_watcher.dart` (~95 lines) with `osNativeDispatchWatcherProvider` (Provider&lt;void&gt; using `ref.listen` on `notificationListProvider(NotificationListQuery(lifecycle: 'incoming'))` with `fireImmediately: true`, diffing against a closure-captured `Set<String>` of seen `notificationId`s). First-tick semantics treats the initial backlog as "already-known" so a relaunch doesn't re-page the operator on every existing incoming. `seen` is updated BEFORE async dispatch to make re-entrant rebuilds safe. Wired into `lib/app.dart` via a single `ref.watch(osNativeDispatchWatcherProvider)` line in `AgentTowerControlPanel.build` so the watcher stays alive for the app's lifetime. `flutter analyze` passes with 0 new issues (one pre-existing `unused_import` warning in `os_native_integration.dart` is captured under T173 cleanup).

**Post-Round-5-analyze follow-up** (added per /speckit-analyze Round 5, 2026-05-25 / T-N2):
- [X] T172 [P] **Bench-verified analyze regression gate.** Add `apps/control_panel/tools/bench_verify.sh` that runs inside the flutter-bench Docker container: `flutter pub get && dart run build_runner build --delete-conflicting-outputs && flutter analyze --fatal-errors && flutter test test/core test/features`. The script exits 0 only when all four steps pass; non-zero on any failure. Wire into the T154-T157 verification suite (or a standalone CI workflow) so language-API drift (e.g. the Riverpod 2.x vs 3.x `ref.mounted` mismatch fixed in commit `8e8e629`) is caught at PR time rather than at packaging (T148) or post-merge. Closes Round-5 analyze T-N2. **Done 2026-05-25**: ~70-line bash script with `set -euo pipefail`, auto-resolves project root from `BASH_SOURCE`, refuses to run outside `apps/control_panel/` (exits 2 with a clear error). Honors `FLUTTER` env-var override (defaulting to `flutter` on PATH; bench Docker should set it to `/opt/flutter-3.27.0/bin/flutter`) and `SKIP_BUILD_RUNNER` for fast analyze-only smoke runs. `bash -n` syntax check passes. CI/workflow wiring deferred to operator.

**Post-T149/T150/T164 follow-up** (added 2026-05-25 during /speckit-implement Phase 9 from test-author surfacings):
- [X] T173 [P] **Test-helper + production-widget hardening (consolidated from T149/T150/T164 helpers + T161/T162/T163/T170 test surfacings + T165 i18n cleanup).** Cleanups surfaced across Phase 9 batches 2-5. **Done 2026-05-25** — all 8 sub-tasks (a)-(h) landed as scoped below. The Fixtures.event vs Event-model shape alignment that was originally bundled under (a)'s introductory sentence is a separate concern from the inference-warning fix and is tracked end-to-end as T176 below; T173(a)'s scope is the inference-warning fix only.
  - (a) **Fix the 3 `inference_failure_on_collection_literal` warnings at `apps/control_panel/test/helpers/fixture_builders.dart:363`** — the `'recent': recent ?? const {'events': [], 'queue': [], 'routes': []}` default in `Fixtures.dashboardResult` (NOT `Fixtures.event`). **Done 2026-05-25**: added explicit `<String, List<dynamic>>` + `<dynamic>[]` annotations; warnings cleared under `flutter analyze`. The separate Fixtures.event vs Event-model shape mismatch (T164's events-view test had to splice event JSON inline) is tracked end-to-end as T176 below — it's a contract-vs-model decision, not a polish item.
  - (b) Add stable `Key`s to widgets that test files currently locate via label-text: `_drivingMasterRow` in project_card.dart, the empty-state `HealthyEmptyStateView` in attention_queue_view.dart, the `_AttentionRow` avatar, MarkdownViewer's rejection-snackbar, PanesView's per-state `TextButton.icon` affordances, HealthView's per-subsystem cards, **and `_SubViewStrip` ChoiceChips** (`Key('subview-chip-$id')` — looked up 6× per walk in `us1_adopt_and_operate.dart`). Decouples tests from UI copy changes.
  - (c) Update spec round-3 clarification or `attention_queue_view.dart` to align on empty-state copy ("All clear — no actionable items" vs the current "No actionable attention items.") — they currently disagree; test asserts the production string.
  - (d) **Consolidate `tester.pumpUntil` polyfill** into `apps/control_panel/test/helpers/pump_until.dart`. Both `integration_test/runtime_states.dart` (T162) and `integration_test/us4_drift.dart` (T170) inline-polyfilled the same helper because `WidgetTester.pumpUntil` does NOT exist on Flutter 3.27.0. Single shared helper avoids drift between two copies.
  - (e) Verify `Fixtures.agent()` payload field name for log attachment. T163 splices `'log_attachment': <wireValue>` into the JSON map via `toJson()` because the builder doesn't expose it; confirm the real daemon's `AdoptedAgent` shape uses the field name `log_attachment` (not `logAttachment` etc.) and extend `Fixtures.agent` to include it natively.
  - (f) **Unify "Retry connection" copy** across surfaces — the dashboard's `_OutageState`/`_DashboardError` (lib/features/agent_ops/dashboard/dashboard_view.dart:230,256) uses the full SC-010 phrasing; the reusable `OutageStateView` in `lib/ui/widgets/runtime_state_views.dart:120` shortens to just "Retry". Pick one and apply everywhere; pulls the inconsistency T162 caught into a single canonical button label.
  - (g) Normalize the i18n import-path style across the 27 files edited in T165 — the notifications agent used relative path `../../core/l10n/app_localizations.dart` while the attention + project_specs + testing_demo agents used the package: form. Both compile; pick canonical style (relative is the prevailing convention in this codebase per the rest of the imports).
  - (h) Clean up the 4 pre-existing `unused_import` warnings flagged by `flutter analyze` post-T165: `lib/core/daemon/app_client.dart:4` (`errors.dart`), `lib/core/persistence/ux_state_repository.dart:3` (`dart:io`) + line 5 (`common_enums.dart`), `lib/features/agent_ops/providers.dart:3` (`app_client.dart`), and `lib/features/notifications/os_native_integration.dart:8` (`providers.dart`).

- [ ] T174 [P] **DaemonSession re-bootstrap-on-retry + mock-daemon stateful mutations.** Two architectural gaps surfaced by the Phase-9-batch-5 integration tests, both worth a coordinated change rather than test-side workarounds:
  - (a) **Lib-side: "Retry connection" should re-bootstrap the session, not just invalidate the dashboard provider.** T162's outage/recovery test had to manually re-bootstrap `DaemonSession` after tapping "Retry connection" because the existing handler only invalidates the dashboard provider. The session's underlying socket stream was closed by the daemon-kill, so re-fetching from the existing session fails — a real operator would also have to relaunch the app today. **API contract for the new method (per /speckit-analyze Round 8 C1):** add `Future<void> reBootstrap()` to `DaemonSession` with the following behavior: (i) close the current `SocketClient` connection; (ii) reset the in-memory `app_session_token` to null; (iii) open a fresh `SocketClient` to the configured socket path; (iv) re-issue `app.hello` and store the new `app_session_token`; (v) emit a `SessionEvent.reBootstrapped` on the broadcast stream so dependent providers can invalidate. **Pending-request semantics:** any `Future` returned by `AppClient.*` methods that was in-flight at the moment of `reBootstrap()` MUST complete with `SocketDisconnectedError` (do NOT silently retry on the new session — the caller's idempotency_key may or may not be replayable; that's a caller decision). The retry-handler call-sites (`lib/features/agent_ops/dashboard/dashboard_view.dart:230,256` plus any sibling retry buttons) should `await ref.read(daemonSessionProvider.notifier).reBootstrap()` THEN invalidate the dashboard provider. **Maintenance dependency (per Round 8 A1):** when this lands, update T162's "Real architectural gap — flagged below" closing note in `tasks.md` to `"Closed by T174(a) on <date>"` so the spec doesn't claim the gap is still open.
  - (b) **Mock-daemon side: `app.handoff.submit` should mirror the submitted `generated_prompt_text` into subsequent `app.handoff.list`/`app.handoff.get` results.** T169 cannot fully close its non-tautological assertion until the mock can prove the submitted bytes round-tripped. Minimal change: in `apps/control_panel/test_harness/mock_daemon/server.py` `handle_client`, when `method == 'app.handoff.submit'`, splice `req['params']['draft']['generated_prompt_text']` into `response['result']['row']['generated_prompt_text']` before returning. Same shape for `app.handoff.list`/`app.handoff.get` would need per-session state. Once landed, swap the "soft probe" in `integration_test/us3_handoff_flow.dart` from `isNotEmpty` to `equals(previewTimeRender)` for the full T169 closure.

- [X] T176 [P] **Fixtures.event vs Event model shape decision (deferred from T173(a)).** **Done 2026-05-25** via option (b) — added 3 `@JsonKey(name: ...)` mappings to `lib/domain/models/event.dart`: `eventType` ↔ `event_class`, `observedAt` ↔ `emitted_at`, `excerpt` ↔ `summary`. Added `// ignore_for_file: invalid_annotation_target` (standard freezed escape hatch for the constructor-parameter `@JsonKey` false-positive). Regenerated `event.freezed.dart` + `event.g.dart` via `flutter pub run build_runner build --delete-conflicting-outputs` (unfiltered run regenerated 74 files across all freezed/json_serializable models — expected). Confirmed parser now reads wire keys directly. T164's events-view widget test inline-JSON splice removed; now uses `Fixtures.event(...)`. `Fixtures.event()` signature unchanged (param names already matched wire shape). `flutter analyze` passes with 0 new errors. `Fixtures.event(...)` in `apps/control_panel/test/helpers/fixture_builders.dart:183` emits FEAT-011 wire fields (`event_class` / `emitted_at` / `summary`) but the `Event` freezed model in `apps/control_panel/lib/domain/models/event.dart` expects (`event_type` / `observed_at` / `excerpt`). T164's events-view widget test had to build the event JSON inline to satisfy the model parser. Three valid fixes (operator decision): (a) **align the Event model to the FEAT-011 wire shape** — rename freezed fields + regenerate `.freezed.dart`/`.g.dart`. Risky if other call-sites depend on `eventType`/`observedAt`/`excerpt` names. (b) **add `@JsonKey(name: '...')` mappings** to the Event model — keeps Dart-side names + accepts the wire shape via JSON parsing. Lowest-risk fix; needs regen. (c) **update Fixtures.event to produce model-shape JSON** — keep the existing wire-shape builder as `Fixtures.eventWire` and add `Fixtures.eventModel` for tests parsing through the model. Lowest-disruption test-side fix. Recommend (b) per "the daemon contract is canonical, the model should match it via mappings" reading of FEAT-011.

- [ ] T175 [P] **`project_specs.open_handoff_flow` palette command is misnamed — add real HandoffFlow entry-point.** Per T168's investigation, `apps/control_panel/lib/features/project_specs/module.dart:135` registers a palette command named `open_handoff_flow` whose handler `Navigator.pushNamed`s to `current_work` — it does NOT open `HandoffFlow`. The only in-app surface that opens `HandoffFlow` from regular navigation is `DriftRepairHandoffLauncher` (via `openHandoffFlow`); the palette entry promises something the rest of the UI doesn't deliver. **Decision committed (per /speckit-analyze Round 8 B1, 2026-05-25):** go with option (b) — add a real `HandoffFlow` entry-point to the Project and Specs workspace (e.g. a "New handoff" button on Current Work that calls `openHandoffFlow(...)`) AND keep the palette command name `open_handoff_flow` (which then matches its behavior). This is the more useful UX fix per the spec's FR-036 multi-step flow expectations. The rename-only option (a) is rejected because it leaves the operator with no non-drift-repair path into the handoff flow at all.

**Post-Round-8-analyze follow-up** (added 2026-05-25 from /speckit-analyze Round 8 E1):
- [X] T177 [P] **Extend FR-067 i18n sweep to dashboard/shell/runtime_state_views (Round 8 E1).** **Done 2026-05-25**: 70 strings extracted across 6 files; `en.arb` grew 312 → 382 value keys; `flutter analyze` passes with 0 new errors. Per-file: `runtime_state_views.dart` (13 keys — 3 surface-specific Retry buttons + 4 message-label pairs + 1 contract-detail with placeholders), `dashboard_view.dart` (35 keys — section titles + stat labels + runtime-state labels + outage/error variants), `app_shell.dart` (9 keys — 4 nav-rail + 4 workspace AppBar + 1 sub-view placeholder with `{workspace}`/`{subViewId}` interpolation), `global_banner.dart` (3 keys), `project_switcher.dart` (2 keys), `version_display.dart` (8 keys). Structural changes: lifted `_workspaceLabel`/`workspaceLabel` to take `AppLocalizations` arg, dropped `const` on `NavigationRail.destinations` outer list + `project_switcher.dart` Dialog (switched builder to capture `dialogCtx` so `AppLocalizations.of()` resolves inside the modal subtree), `version_display.dart` loading-branch `const`-drop. Skipped: single-glyph defaults (`'—'`, `'?'`), provider-only files (`runtime_state_provider.dart`). Together with T165, FR-067 is now fully covered across every Phase 3-9 widget. Closes Round 8 E1. When T165 uncommented `AppLocalizations.delegate` in `app.dart`, FR-067 ("ALL user-facing strings MUST be routed through a single localization layer") became a verifiable invariant. T165's original scope explicitly excluded `lib/features/agent_ops/dashboard/**`, `lib/features/shell/**`, and `lib/ui/widgets/runtime_state_views.dart` (see T165's "Scope carve-out" note). Those uncovered files now visibly violate FR-067 in production: 5× `Text('Retry connection')` literals across `lib/ui/widgets/runtime_state_views.dart:120,203,282` + `lib/features/agent_ops/dashboard/dashboard_view.dart:230,256` (the latter pair just unified by T173(f)), plus "Release notes" / "Version unavailable" in `lib/features/shell/version_display.dart:121,132`. **Scope:** every `Text('literal')` / `Tooltip(message: ...)` / `SnackBar(content: Text(...))` / `AppBar(title: Text(...))` under those 3 path globs. **Approach:** mirror T165's strategy — extract per-call-site key into `en.arb` with auto-generated `@key` description, wire each call-site to `AppLocalizations.of(context).<key>`, regenerate `app_localizations.dart` via `flutter gen-l10n`, run `flutter analyze` to verify 0 new errors. Closes FR-067 fully (T165 + T177 together cover every Phase 3-8 widget). Closes Round 8 E1.
