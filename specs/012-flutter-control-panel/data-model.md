...
- **Master qualification (FR-071)**: the `MasterSummary` projection from an `AdoptedAgent` is gated on both `role == master` AND `capability ∈ masterClassCapabilities`. The master-class set is fetched from the daemon (R-19) and cached for the session. Construction MUST go through `MasterSummary.tryFromAgent(...)` rather than the raw freezed factory so the gate is unbypassable (swarm-review H-G1, 2026-05-24).
...
- **Enum wire-format invariant (CR-1 / 2026-05-24)**: every multi-word enum in `lib/domain/models/` carries `@JsonEnum(valueField: 'wireValue')` so `json_serializable` emits the snake_case / hyphenated wire string from the enum's `wireValue` field rather than the Dart identifier. Without this, `fromJson` on any payload containing a multi-word enum throws `CheckedFromJsonException`. New enums MUST add the annotation; the convention is asserted at codegen time and surfaced by the integration tests.
- **Wire-shape list filters (CR-3 / 2026-05-24)**: list-method filters MUST nest under a `filters: {…}` object on the request per FEAT-011 `app-methods.md §list-request-shape` (line 169-177); the top-level slot is reserved for `limit`, `cursor_next`, and `order_by`. The shared `AppClient._list` helper enforces this for every consumer.
