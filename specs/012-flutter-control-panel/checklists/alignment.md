...
- [ ] CHK034 - Does research R-14 (200 ms p95 latency threshold) match observability.md CHK003 expectation (per the Round-1 observability checklist) and FR-074's "documented threshold"? [Consistency, Research R-14 / Observability CHK003 / Spec §FR-074] → **OPEN per Round-2 finding A1 / F-A3**: research R-14 names 200ms p95 and tasks.md T023 enforces it, but the spec FR-074 wording stays abstract ("documented threshold"). Cosmetic-only; addressed only if spec authoring style changes.
...
- [ ] CHK037 - Does research R-12 (release feed) reconcile with security.md CHK029 / api-contract.md CHK039 (FR-001 + SC-009 vs FR-068 outbound HTTPS)? [Consistency, Research R-12 / Spec §FR-001/§FR-068/§SC-009] → **OPEN per Round-2 finding F-A4**: FR-001 carve-out for the FR-068 release-feed check is interpretive ("to populate its views" qualifier), not explicit. T155 (security trace task) enforces the runtime invariant. Cosmetic spec polish.
...
- [ ] CHK047 - F5: Is the master/agent state relationship from FR-030 reflected in data-model.md §1.3 (Master Summary as a view over Adopted Agent)? [Consistency, Data-model §1.3 / Spec §FR-030/§FR-071] → **OPEN per Round-2 finding F-A5**: data-model.md §1.3 calls MasterSummary "a view, not a separate entity" but does not explicitly state "currentStatus projects over agent state". The relationship is in spec FR-030. Cosmetic data-model polish.
...
- [ ] CHK058 - Are there entities in data-model.md that have no corresponding mention in spec.md Key Entities? [Coverage] → **OPEN per Round-2 finding F-A9**: ResolvedWorkItem (data-model §1.7) and HelperPolicy/HelperPolicySnapshot (§1.8) are first-class entities in data-model but absent from spec.md Key Entities. Cosmetic spec polish (would add 2 bullets to Key Entities).
- [X] CHK059 - Are there quickstart steps that exercise behavior not specified in spec.md or US1 scenarios? [Coverage]
- [ ] CHK060 - Are there Spec assumptions or Out-of-Scope items that have no acknowledgment in plan.md or research.md? [Traceability] → **OPEN per Round-2 finding F-A13**: Spec Assumption about doc-path-discovery (conventional paths `docs/product-requirements.md` etc.) is mentioned in plan briefly but not explicitly handled in plan or research. Defer; not blocking.
