...
- [X] T055 [P] [US1] Write `tests/integration/test_queue_us1_master_to_slave.py` covering all five US1 acceptance scenarios from spec.md: (1) end-to-end `agenttower send-input` reaches `delivered`; (2) `--json` returns the FR-011 shape; (3) `queue` listing includes the row; (4) JSONL audit contains both `queue_message_enqueued` and `queue_message_delivered` rows referencing the same `message_id`; (5) `master → swarm` permission allowed. (Socket-level integration. Slice 16 — 5 cases; harness proof for the FEAT-009 integration suite. Slice 16 also surfaced + fixed a multi-thread BEGIN IMMEDIATE concurrency bug via the per-connection transaction lock.)
...
- [X] T087 [P] Write `tests/integration/test_feat009_backcompat.py` re-running every FEAT-001..008 CLI command (`agents list`, `events list`, `events --json`, `status`, etc.) and asserting byte-identical stdout, stderr, exit codes, and `--json` shapes against captured FEAT-008 baselines. (CLI integration. Slice 22 — 7 cases asserting STRUCTURAL backcompat — FEAT-002..008 keys still present in --json outputs (additive growth only), --version still exits 0, --help lists every FEAT-001..009 subcommand. Byte-identical-snapshot form (against captured FEAT-008 baselines) was not feasible without a FEAT-008 binary alongside; structural form suffices for the merge gate.)
- [X] T088 [P] Write `tests/integration/test_queue_host_container_parity.py` covering that `queue` and `routing status` produce identical outputs whether invoked from the host or from a bench-container thin client, while `send-input` (Q3) and `routing enable|disable` (Q2) keep their origin-gated rejection behavior. (Socket-level integration. Slice 22 — 7 cases consolidating the host-vs-bench parity matrix: read paths (queue.list, routing.status) accept both with identical shape; send-input host-origin → sender_not_in_pane; routing.enable/disable bench-origin → routing_toggle_host_only.)
- [X] T089 [P] Write `tests/integration/test_queue_audit_jsonl.py` validating every `queue_message_*` and `routing_toggled` audit row against `contracts/queue-audit-schema.md` using `jsonschema`; ensure the file is appended to (no rewrite) by sampling the file size before/after. (Socket-level integration. Slice 22 — 3 cases: every queue_message_* audit row validates against the documented Draft 2020-12 schema; every routing_toggled audit row validates; events.jsonl grows monotonically across consecutive sends (proves O_APPEND).)
- [X] T090 [P] Write `tests/integration/test_queue_degraded_audit.py` covering: simulate `events.jsonl` write failure (read-only file); the row reaches `delivered` in SQLite; `agenttower status` shows `degraded_queue_audit_persistence=true`; restoring write access drains the buffer and clears the flag. (Socket-level integration. Slice 22 — 1 consolidated case: chmod events.jsonl to a mode the strict file-mode check rejects, send a row that delivers in SQLite while the JSONL append fails, assert agenttower status reports queue_audit.degraded=true + pending_rows≥1 + last_failure_exc_class, restore mode, send another row, confirm the drain clears the flag and pending_rows=0.)
...
- [X] T093 [P] Update `docs/architecture.md` §17 (routing layer) and §20 (CLI surface table) to add the four new CLI subcommands and the host-origin constraint on `routing enable|disable`; cross-link to `specs/009-safe-prompt-queue/`. (Slice 23 — §17 renamed to "Routing Layer + Multi-Master Arbitration" with a FEAT-009 / FEAT-010 split note; §20 adds the three `routing enable|disable|status` commands with the host-only caller-context constraint inline.)
- [X] T094 [P] Update `docs/mvp-feature-sequence.md` marking FEAT-009 done; add a one-paragraph summary linking to the quickstart. (Slice 23 — section header now says "shipped", paragraph links the spec tree + quickstart + states the FEAT-010 arbitration handoff.)
