...
- [ ] T032 [P] [US4] Add integration tests for `agenttower stop-daemon` in `tests/integration/test_cli_stop_daemon.py`: clean stop (default + `--json`); no-reachable-daemon case (US4 acceptance #3) returns exit 2 with documented stderr and asserts no Docker, tmux, shell, or fallback process is invoked (FR-020 / analyze finding E1).
