...
**Decision**: Pure-Python validation via `envelope.py` and `errors.py` — no `pydantic`, no `jsonschema`. Success envelopes are built by `envelope.success(result=None)` (default `result={}`) and failure envelopes by `envelope.failure(code, message, details=None)`. Both helpers stamp `app_contract_version` automatically. The `errors.py` module exposes a registry `DETAILS_REQUIRED_KEYS: dict[str, frozenset[str]]` enumerating required keys per code; `validate_details(code, details)` (called from `envelope.failure()`) raises a daemon-side `ContractViolation` exception if a handler emits a malformed failure (unknown code, missing required key, non-object `details`, or non-empty `details` on a code not in the registry per FR-034a). The dispatcher's `_wrap_handler` in `app_contract/dispatcher.py` catches `ContractViolation` and any other handler exception, mapping both to the FEAT-011 `internal_error` envelope so the wire always sees a structurally-valid FEAT-011 envelope shape (FR-033 invariant).

**Rationale**: The contract test suite is the system of record for envelope shape; adding `pydantic` / `jsonschema` to the runtime introduces a heavyweight dependency for a closed contract where every shape is known in advance. The closed-set codes and required-key registry are small (27 codes, 15 registry entries with structured details) — a hand-written validator is more readable and faster than a schema engine. Contract tests still independently verify response shapes from the outside.
