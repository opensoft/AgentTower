...
**Testing**: pytest. The target test layout has contract tests under `tests/contract/test_app_*.py` using synthetic Unix-socket clients (no `agenttower` subprocess invocation — see SC-001) and integration tests under `tests/integration/test_story*.py`. The current PR ships in-process unit tests at `tests/unit/test_app_*.py` (smoke + foundational + view_models + scan_handlers + reads + adopt — 8 files, ~190 tests covering FR/SC assertions at the function-call level) plus socket-level integration tests at `tests/integration/test_story1_dashboard_bootstrap.py` (Story 1: 17 tests including SC-002 latency + FR-003a/b wire framing + SC-008 token redaction) and `tests/integration/test_story2_adopt_roundtrip.py` (Story 2: 4 tests including SC-004 + FR-028a wire-level identity match + FR-044 audit attribution). Migration of the unit-level assertions into the structured `tests/contract/` layout is tracked as tasks T019..T022 and is part of the US1 polish slice; it adds organization but no coverage. Existing FEAT-002..010 test suite remains untouched.
