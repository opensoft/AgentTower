...
1. **State-bucket aggregators** in `app_contract/dashboard.py` — derive `counts.panes.by_state` (4-key `PaneState`) and `counts.agents.by_state` (5-key `AgentState`) from the same daemon-state rows that already back the v1.0 `counts.panes.{total,registered,unregistered}` fields, alongside the existing v1.0 `_pane_counts` / `_agent_counts` helpers. Bucket assignment uses the FEAT-004/011 semantics fixed by Clarifications Q1–Q3 (no new heuristics). The aggregators satisfy the FR-019 *partition* invariant within a single call by construction (the four buckets derive from one internal `total` read with `dau` as the remainder). The strict total-sum cross-check against the v1.0 `counts.panes.total` is NOT transactionally pinned across the two helpers — per FR-018 counts are read sequentially with no global lock and slight inter-surface inconsistency is acceptable, so the strict total-sum holds per-snapshot and may transiently diverge if FEAT-004 discovery commits a pane insert/delete between the v1.0 count read and the bucket read (self-corrects on the next poll). (`view_models.py` is intentionally NOT used — that module is pure entity-row projection with no DB I/O; aggregators live alongside the v1.0 count helpers per analyze D-DRIFT-1 correction.)
...
- Extend `tests/unit/test_app_dashboard.py` for the v1.1 fields, FR-019 cross-check invariants (post-R3 one-sided), FR-020 agent partition, FR-021 compute-failure null fallback, and the v1.0 envelope shape regression — this is the wire-envelope-level coverage.
- New `tests/unit/test_pane_state_buckets.py` and `tests/unit/test_agent_state_buckets.py` — dedicated unit tests for the bucket aggregators directly: PaneState assignment + §PB priority (Clarifications Q1) and the FR-020 agent partition + FR-006 orthogonality, respectively. These test the `_compute_*_buckets` helpers in isolation; `test_app_dashboard.py` above asserts the same logic at the envelope level.
...
- New `tests/unit/test_v1_0_compat.py` — FEAT-014 SC-004 regression: boots a v1.1-advertising daemon and replays the FEAT-011 v1.0 contract suite via `pytest … -m 'not v1_1'`, asserting every pre-existing v1.0 assertion still passes unchanged (T023). The subprocess replay is opt-in via `AGENTTOWER_RUN_SC004_REGRESSION` (set in CI's unit-gate step) so it doesn't double-run the contract suite on every local `pytest tests/unit` (post-swarm M11).
- Extend `tests/integration/test_story1_dashboard_bootstrap.py` for FEAT-014 SC-006 (dashboard p95 latency ≤ 500 ms still holds with v1.1 fields populated — same latency budget FEAT-011 published as its SC-002, renumbered to SC-006 under FEAT-014's spec) and the no-new-I/O-surface check that falls out of FEAT-014's additive-minor design (no separate FEAT-014 SC; covered by FR-022 plus the structural-absence note in tasks.md §Notes).
...
**Performance Goals**: FEAT-014 re-specifies the FEAT-011 dashboard latency budget as **FEAT-014 SC-006: p95 ≤ 500 ms** at the FEAT-011 fixture scale (Clarifications R1 Q9), and — unlike FEAT-011 SC-002 (worst-case 5/5 trials, which held even in degraded states) — waives the p95 assertion during `subsystem_degraded`. The 500 ms figure is shared, but the percentile (p95 vs worst-case-5/5) and the degraded-state posture differ; do not treat them as identical assertions. The FEAT-011 "warm < 100 ms" figure is an `app.readiness`/`scan.status` informational SLA, NOT part of the dashboard SC-002 budget, and is not inherited here. Per the cost model in Research §CO, expected additive cost is < 5 ms at the FEAT-011 fixture scale. When this plan or its tasks reference an `SC-NN`, the FEAT-014 numbering is authoritative unless the prefix `FEAT-011 SC-NN` is explicit.
...
└── test_v1_0_compat.py          # NEW — FEAT-014 SC-004 regression: boots a v1.1-advertising
                                 #   daemon and replays the FEAT-011 v1.0 contract suite via
                                 #   `pytest … -m 'not v1_1'` so FEAT-014's additive assertions
                                 #   are excluded; asserts every pre-existing v1.0 assertion
                                 #   still passes unchanged. Created by T023.

tests/integration/
└── test_story1_dashboard_bootstrap.py  # EXTENDED — FEAT-014 SC-006 latency p95 ≤ 500 ms
                                        #   with v1.1 fields populated (same numeric budget
                                        #   FEAT-011 labels SC-002); no-new-I/O-surface check
                                        #   per FR-022 (no separate FEAT-014 SC); one
                                        #   end-to-end fixture per User Story acceptance #1
                                        #   (US1, US2, US3 first scenario).
```

**Test file naming note**: The `test_story1_dashboard_bootstrap.py` filename is preserved from FEAT-011 for continuity with that feature's per-story test convention. Under FEAT-014 the file houses one end-to-end acceptance scenario per user story (US1, US2, US3) plus the cross-cutting FEAT-014 SC-006 latency assertion and the no-new-I/O-surface check (FR-022) — i.e., the "story1" in the filename refers to FEAT-011's Story 1 (the dashboard-bootstrap journey), not to FEAT-014's US1. A future feature minor may rename to `test_acceptance_scenarios.py` once FEAT-011's per-story pattern diverges further; for now the rename would cascade through multiple speckit task references and is deferred.
