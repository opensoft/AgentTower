...
- Q: What signal determines `active` vs `inactive` for a registered agent? → A: Agent's container `state == "active"` AND the agent's own `active` flag set → `active`; container `inactive`/`degraded_scan` OR the agent's own `active` flag unset (FEAT-006 pane cascade) → `inactive`.
...
### Session 2026-05-25-r3

Post-MVP-implementation Round 3 — resolves the FR-019 vs Research §PB inconsistency surfaced by analyze finding **F-019-PB-1** (HIGH) during MVP implementation and tracked in GitHub issue #28. Single targeted question.

- Q: FR-019 vs Research §PB priority for registered-pane-on-inactive-container — which takes precedence when v1.0 `counts.panes.registered` counts a pane (its registered agent is `active=1`) but §PB priority sends it to `inactive-or-stale` (its container has `active=0`)? → A: **Option B** — loosen FR-019 from strict `==` to `≤` with a documented gap rule. The current MVP implementation (commit `dbd1f3e`) is correct under this resolution; no code change required. Research §PB priority remains canonical. Operationally meaningful: an operator looking at the dashboard during a container shutdown sees the pane flagged "stale," not "registered."

Notes:

- FR-019 wording updated: `discovered-and-registered ≤ v1.0 counts.panes.registered`, with the gap `(v1.0 registered) - dar` equal to the count of registered panes §PB routes to `inactive-or-stale`/`discovery-degraded` (container `inactive`/`degraded_scan`, the pane's own `active` flag unset, or stale `last_seen_at`). The sum of the three "non-registered" buckets gains the same gap on the other side: `dau + ios + dd ≥ v1.0 unregistered`. The total-sum invariant (all four buckets sum to v1.0 `total`) remains strict.
- `data-model.md` §PaneState invariants updated to match.
- `research.md` §PB priority order unchanged — it remains the canonical priority rule.
- `_compute_pane_state_buckets` docstring tightened: the "Tracked in #28" pointer is replaced with a "Resolved by §Session 2026-05-25-r3" pointer.
- This closes GitHub issue #28 and analyze finding F-019-PB-1.
...
- **FR-019**: System MUST guarantee that v1.1 `counts.panes.by_state.discovered-and-registered` is **less than or equal to** v1.0 `counts.panes.registered`, that the sum of `discovered-and-unmanaged` + `inactive-or-stale` + `discovery-degraded` is **greater than or equal to** v1.0 `counts.panes.unregistered`, and that the sum of all four v1.1 buckets is **equal to** v1.0 `counts.panes.total`. The gap between v1.0 `registered` and v1.1 `dar` equals the count of registered panes that the Research §PB priority rule routes to `inactive-or-stale` or `discovery-degraded` instead of `discovered-and-registered` — i.e. panes whose owning container is `inactive`/`degraded_scan`, whose own `active` flag is unset (FEAT-004 reconciliation marked the pane inactive while its container stayed active), or whose `last_seen_at` is stale (the `last_seen_at` half is deferred until upstream scan-timestamp wiring lands). The same gap appears on the other side of the cross-check (raising `dau + ios + dd` above v1.0 `unregistered`), and the total-sum invariant remains strict because the v1.1 partition is exhaustive. (Loosened from strict `==` to one-sided invariants in Clarifications §Session 2026-05-25-r3 Q1 — see the rationale there and the original spec inconsistency tracked in GitHub issue #28.) For the purpose of these cross-checks, an agent classified as `partially_configured` (per FR-020) STILL counts as "registered" for its pane's `discovered-and-registered` bucket — incomplete agent configuration affects the agent's bucket only, not the pane's bucket. The pane bucket is a property of the pane and its container, not of the agent's configuration completeness.
- **FR-020**: System MUST treat a registered agent as `active` when its container `state == "active"` **AND the agent's own `active` flag is set**, and as `inactive` when its container is `inactive`/`degraded_scan` **OR the agent's own `active` flag is unset** (FEAT-006 cascaded it inactive when its bound pane went inactive even while the container stayed active). System MUST treat the agent as `partially_configured` — mutually exclusive with `active`/`inactive` — when one or more of `role`, `capability`, or `label` is missing/empty/`unknown`, such that `active` + `inactive` + `partially_configured` equals total agents. The `log-attached`/`log-detached` buckets remain orthogonal and may overlap any of the three.
...
- **"Best-effort" canonical definition** (anchors FR-025, FR-026, FR-027): a "best-effort" response is one in which the daemon emits the full v1.1 envelope with every required key present and well-typed, populated with whatever data the still-up subsystems can produce, never substituting an error envelope or omitting a v1.1 key. The three FRs that use the term cover three distinct failure surfaces: FR-025 (aggregator compute failure — buckets emit `0` per key); FR-026 (subsystem degradation — counts are not suppressed, recommendation engine signals via `subsystem_degraded`); FR-027 (latency-budget overrun — response is returned with measured-but-overrun latency plus a WARN log line). In every case, "best-effort" means **the contract shape is preserved unconditionally** and the failure mode is surfaced through the recommendation or the log, not by mutating the envelope.
