...
- FR-019 wording updated: `discovered-and-registered ≤ v1.0 counts.panes.registered`, with the gap `(v1.0 registered) - dar == panes whose registered agent is on an inactive or degraded container`. The sum of the three "non-registered" buckets gains the same gap on the other side: `dau + ios + dd ≥ v1.0 unregistered`. The total-sum invariant (all four buckets sum to v1.0 `total`) remains strict.
...
- **FR-019**: System MUST guarantee that v1.1 `counts.panes.by_state.discovered-and-registered` is **less than or equal to** v1.0 `counts.panes.registered`, that the sum of `discovered-and-unmanaged` + `inactive-or-stale` + `discovery-degraded` is **greater than or equal to** v1.0 `counts.panes.unregistered`, and that the sum of all four v1.1 buckets is **equal to** v1.0 `counts.panes.total`. The gap between v1.0 `registered` and v1.1 `dar` equals the count of panes whose registered agent is on an inactive or `degraded_scan` container; the Research §PB priority rule routes those panes to `inactive-or-stale` or `discovery-degraded` rather than `discovered-and-registered`. The same gap appears on the other side of the cross-check (raising `dau + ios + dd` above v1.0 `unregistered`), and the total-sum invariant remains strict because the v1.1 partition is exhaustive. (Loosened from strict `==` to one-sided invariants in Clarifications §Session 2026-05-25-r3 Q1 — see the rationale there and the original spec inconsistency tracked in GitHub issue #28.) For the purpose of these cross-checks, an agent classified as `partially_configured` (per FR-020) STILL counts as "registered" for its pane's `discovered-and-registered` bucket — incomplete agent configuration affects the agent's bucket only, not the pane's bucket. The pane bucket is a property of the pane and its container, not of the agent's configuration completeness.
