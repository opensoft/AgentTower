...
- [ ] T021 [P] [US4] Extend the contract version test to cover four sub-assertions, each mapped to its FR for traceability: (a) daemon advertises `app_contract_version == "1.1"` after T002 — **FR-013**; (b) supported-minor-range maximum is `1.1` — **FR-013**; (c) `capability_flags == {}` (no new flag) — **FR-015**; (d) major-version rejection behavior unchanged for client major ≠ 1 — **FR-014** + US4 acceptance #2. The four sub-assertions are bundled in one task because they live in the same file and share fixture setup; mark each with its own pytest test-id so a sub-assertion can be skipped or re-run independently. File: `tests/contract/test_app_versioning.py`.
- [ ] T022 [P] [US4] Add a v1.0-only-reader assertion to the dashboard contract test: a caller that reads only the v1.0 keys (`counts.panes.{total,registered,unregistered}`, `recents`, `hints`) gets back well-typed v1.0 values and is not disturbed by the v1.1 additions appearing in the same envelope (US4 acceptance #1). File: `tests/contract/test_app_dashboard.py`.
- [ ] T023 [P] [US4] Add SC-004 regression as a pytest-fixture-based test (NOT a CI matrix step — keeping the regression inside the same pytest run as the rest of FEAT-014): create `tests/contract/test_v1_0_compat.py` that boots a v1.1-advertising daemon and re-imports / re-exercises the FEAT-011 v1.0 contract assertions, parametrized over the existing `tests/contract/test_app_*.py` modules from FEAT-011. Assert every pre-existing assertion passes unchanged against the v1.1 daemon. File: `tests/contract/test_v1_0_compat.py` (NEW).
...
- [ ] T024 [P] Add an SC-006 latency assertion to the dashboard bootstrap integration test: with all v1.1 fields populated against the FEAT-011 fixture scale (≤ 10 containers, ≤ 200 agents, ≤ 100 routes), `app.dashboard` cold-start-to-response remains within the FEAT-011 **SC-002 budget of ≤ 500 ms** (no-cache, ≥1 container, ≥1 agent fixture). Use the same fixture + measurement methodology as the existing FEAT-011 latency test in `tests/integration/test_story1_dashboard_bootstrap.py`. File: `tests/integration/test_story1_dashboard_bootstrap.py`.
- [ ] T025 [P] Add an SC-005 single-envelope-shape assertion to the dashboard contract test: one v1.1 `app.dashboard` call against a populated daemon returns every new v1.1 field present and correctly typed in a single response envelope. File: `tests/contract/test_app_dashboard.py`.
- [ ] T026 [P] Cross-reference (NOT incorporate) the v1.1 additions from FEAT-014's spec dir into FEAT-011's contract docs. Add an "App Contract Evolution — v1.1 (FEAT-014)" subsection to `specs/011-app-backend-contract/contracts/app-methods.md` containing a one-paragraph summary of the v1.1 additive fields and explicit pointers to `specs/014-app-dashboard-extensions/contracts/dashboard-v1_1.md` (wire shape) and `closed-sets-v1_1.md` (new closed-set values). Repeat the same one-line pointer in `specs/011-app-backend-contract/contracts/closed-sets.md`. Do NOT duplicate v1.1 content into FEAT-011's specs dir — the canonical v1.1 contract docs live under `specs/014-app-dashboard-extensions/contracts/`. Maps to FR-016, SC-007.
