...
- [ ] T019 [US3] Create the new recommendation engine module exposing a **4-symbol public surface** (corrected per analyze M-T019-API + M-T019-RETURN — TDD-revealed by T016's concrete test contract, same pattern as T013's dual-surface fix):
...
- [ ] T020 [US3] Wire the recommendation call into the `app.dashboard` success-envelope assembly. Two sub-steps inside `app_dashboard()`:

  1. **Build a `RecommendationState`** from the same SQLite row sources the existing v1.0 count helpers and v1.1 aggregators use — `_container_counts(ctx)`, `_pane_counts(ctx)`, `_compute_pane_state_buckets(ctx)`, `_route_counts(ctx)`, plus the FEAT-011 readiness probes (`probe_*` results) for `degraded_subsystems`, and "first" deterministic id lookups for `first_active_container_id` / `first_unadopted_pane_id` / `oldest_blocked_message_id` per FEAT-011's normative orderings (Research §CC). The state-building step is intentionally separate from `compute_recommendation` so the latter remains a pure, side-effect-free function over the input dataclass (per T019's signature).
