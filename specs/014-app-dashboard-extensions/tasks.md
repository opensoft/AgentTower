...
- [x] T013 [US2] Create the new skip-counter module exposing a **dual-surface** API (corrected per analyze M-API-SHAPE + M-CONST-CASE — TDD-revealed by T010's concrete test contract):
  - **Class** `SkipCounter` (for test isolation; each test constructs its own instance) with methods `record_skip(now_ms: int) -> None` and `count_in_window(now_ms: int) -> int`. Backed by `collections.deque(maxlen=MAXLEN)` of monotonic-ms integers per Research §RB. Read-time filter is the half-open `now_ms - WINDOW_MS < entry_ms <= now_ms` per Research §CW (strict `>` lower edge + `<= now_ms` upper clamp, see T010 boundary tests). **Thread-safety:** `record_skip` runs on the FEAT-010 routing-worker thread while `count_in_window` runs on a `ThreadingUnixStreamServer` handler thread, so both MUST be guarded by a `threading.Lock`, and the read MUST snapshot the deque (`list(...)`) under the lock before filtering, to avoid `RuntimeError: deque mutated during iteration`. (T010 SHOULD include a concurrency case: a writer thread appending while a reader counts asserts no exception.)
