...
- [ ] T028 [P] [US2] Integration test in `tests/integration/test_story2_auto_prepare_operations.py` covering US2 acceptance scenarios 1–3 (managed pane has role/capability/label/state/log-attach state; output classified + routable through same event surfaces; managed + adopted coexist without separate workflows). Additionally assert: (a) **FR-015 per-pane FIFO + per-layout FIFO ordering** of lifecycle events by recording the event sequence from a layout creation and verifying that all events for any single pane / layout appear in state-transition order; (b) **FR-021 env-var redaction policy** — emit a layout whose launch profile includes env vars with keys `AWS_SECRET_TOKEN`, `MY_KEY`, `OPERATOR_PASSWORD`, `LOG_LEVEL`, `PATH`; after creation assert the JSONL audit record redacts the first three (key-match against `*TOKEN*` / `*SECRET*` / `*KEY*` / `*PASSWORD*`, case-insensitive substring) and preserves the last two **and** the argv + `working_dir` unredacted. **Sub-scope split (Phase 4b/4c)**: Phase 4b (`3271d12`) shipped the building blocks T028 depends on — the background spawn task (T029/T030), per-pane log-attach + launch-immediate-exit failure tests (T026/T027), and the synchronous FR-015 sequence assertion in `test_managed_dispatch.py::test_app_create_emits_synchronous_lifecycle_events`. Phase 4c will write the integration-test file itself once production backend wiring exists so US2 is genuinely end-to-end exercisable (real AgentService.register_agent + LogService.attach_log + FEAT-004 docker-exec). Per N35 resolution, the FR-021 redaction assertion shape is documented in research §R11's "Payload schema reconciliation with FR-021" note: no current event payload carries env / argv / working_dir, so T028's redaction assertion verifies "no env-keyed value appears in any event payload at all" (a stricter form of the FR-021 policy) until/unless a later feature adds diagnostic env fields to a failure event.
